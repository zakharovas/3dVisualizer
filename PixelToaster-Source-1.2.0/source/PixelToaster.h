// Pixel Toaster Library Interface
// Copyright © 2004-2006 Glenn Fiedler
// http://www.pixeltoaster.com

#ifndef PIXEL_TOASTER_INTERFACE
#define PIXEL_TOASTER_INTERFACE
...
#if defined( _WIN32 ) && defined( __DYNAMIC__ )
    #ifdef PIXEL_TOASTER_DLL
    #define PIXEL_TOASTER_API __declspec(dllexport)
    #else
    #define PIXEL_TOASTER_API __declspec(dllimport)
    #endif
#else
    #define PIXEL_TOASTER_API
#endif
...
assert( ! (a == b) );
	...
	PIXEL_TOASTER_API class DisplayInterface * createDisplay();
	PIXEL_TOASTER_API class TimerInterface * createTimer();
	PIXEL_TOASTER_API class Converter * requestConverter( Format source, Format destination );
